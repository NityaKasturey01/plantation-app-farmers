...
import 'conclusion_farmer_consent0.dart';
import 'farmer_consent.dart';
import 'farmer_demand.dart';
import 'package:http/http.dart' as http;

import 'follow_up_1_farmer_cons.dart';



class FarmerConclusion extends StatefulWidget {
  ...
  FarmerConclusion(this.Farmer, this.fid, this.FarmerDemandMap);

  @override
  State<FarmerConclusion> createState() => _FarmerConclusionState();
}

class _FarmerConclusionState extends State<FarmerConclusion> {
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Container(
                                  ...
                                  child: RaisedButton(
                                    ...
                                    onPressed: (){
                                      ...
                                      // );
                                      print("QTYQTY"+trees_qty.toString());

                                      showDialog(
                                        context: context,
                                        builder: (BuildContext context) => new FlutterExample(dropdownvalue1, trees_all, trees_qty, widget.Farmer, widget.fid),
                                      );
                                ...
                                Container(
                                  ...
                                  child: Scrollbar(
                                      child: ListView.builder(
                                        itemCount: trees_all.length,
                                        itemBuilder: (BuildContext context, int index) {
                                          print("Farmer List View");
                                          print(trees_all);
                                          return new Column(
                                            children: <Widget>[
                                              new ListTile(
                                                title: new Text(trees_all[index]),
                                                subtitle: new Text(trees_qty[index].toString()),
                                              ),
                                              new Divider(
                                                height: 2.0,
                                              ),
                                  ...
                                  child: RaisedButton(
                                    ...
                                    onPressed: (){
                                      ...
                                      if(FlutterExample.FarmerDemandMap.toString() == "{}"){
                                        ...
                                        tree.clear();
                                        trees_all.forEach((element) {
                                          tree.add(null);
                                        });

                                        Navigator.push(context, MaterialPageRoute(builder: (context) =>
                                            FarmerConclusionFConsent(widget.Farmer, "true", widget.fid, tree_type,
                                                trees_all, trees_qty, null, tree),
                                        ),);
                                        // Navigator.push(context, MaterialPageRoute(builder: (context) => FarmerDemandFConsent(widget.year,
                                        //     widget.status, widget.date, widget.district, widget.block, widget.village, widget.farmer, widget.aadhar
                                        //     , widget.phone, widget.gender, dropdownvalue1, FlutterExample.FarmerDemandMap, null, widget.userID),
  ...
  void initState() {
    createBox();
    print("Farmer Demand Data iniit");
    print(widget.FarmerDemandMap);
    setState((){
      if(!widget.FarmerDemandMap.isEmpty){
        trees_all.clear();
        trees_qty.clear();
        widget.FarmerDemandMap.keys.forEach((key) {
          print(key);
          trees_all.add(key);
        });
        widget.FarmerDemandMap.values.forEach((value) {
          print(value);
          trees_qty.add(value);
        });
      }else{
        trees_all = selected_tree;
        trees_qty = qty;
      }
      print(trees_all);
      print(trees_qty);
    ...
    box1?.get("demandFID").asMap().forEach((index, element) {
      ...
      if(element.toString() == widget.fid.toString() && box1?.get("demandUserID")[index].toString() == box2?.get("email")){
        ...
        setState(() {
          ...
          //trees_all.add(box1?.get("demandList")[index].toString());
          //trees_qty.add(box1?.get("demandqty")[index].toString());
...
class ListTileItem extends StatefulWidget {
  String title;
  int qty;
  ListTileItem({required this.title, required this.qty});
  @override
...
class _ListTileItemState extends State<ListTileItem> {
  ...
  void initState() {
    ...
    super.initState();
    setState(() => _itemCount=widget.qty);
    FlutterExample.FarmerDemandMap[widget.title] = widget.qty;
...
class FlutterExample extends StatelessWidget {
  ...
  String demandCat;
  var demandCatList, qtyList;
  String farmer;
  int fid;

  FlutterExample(this.demandCat, this.demandCatList, this.qtyList, this.farmer, this.fid);
  static Map<String, int> FarmerDemandMap = {};


  @override
  Widget build(BuildContext context) {
    print("QTYQTY"+qtyList.toString());
    return AlertDialog(
      ...
      content: new SizedBox(
        ...
        child:ListView(
          children: new List.generate(demandCatList.length, (i)=>new ListTileItem(
              title: demandCatList[i], qty: int.parse(qtyList[i].toString())
          )),
        ),),
      actions: <Widget>[
        new FlatButton(
          onPressed: () {
            print("Farmer Data Demand: ");
            print(FarmerDemandMap);
            Navigator.of(context).pop();
            Navigator.of(context).pop();
            Navigator.push(context, MaterialPageRoute(builder: (context) => FarmerConclusion(
                farmer, fid, FarmerDemandMap
            ),),);
